...
Lighting directional_light, point_light, spot_light;
float shininess = 64;
...
Magnification magnification = mag_nearest;
Minification minification = min_nearest;
...
GLuint iLocV;
GLuint iLocM;
GLuint iLocNormalTransform;
GLuint iLocAmbient;
GLuint iLocDiffuse;
GLuint iLocSpecular;
GLuint iLocDiffuseIntensity;
GLuint iLocLightPos;
GLuint iLocViewPos;
GLuint iLocIsPerPixel;
GLuint iLocligtingMode;
GLuint iLocShininess;
GLuint iLocCutoff;
GLuint iLocTexture1;
GLuint iLocisEye;
GLuint iLocEyeOffset;
...
void setGLVec3(GLfloat* glm, Vector3 m) {
	for (int i = 0; i < 3; i++) {
		glm[i] = m[i];
	}
}
...
void RenderScene(int per_vertex_or_per_pixel) {	
	int isPerpixel = per_vertex_or_per_pixel;
	Vector3 modelPos = models[cur_idx].position;
	
	Matrix4 T, R, S, normal_transform;
	...
	normal_transform = model_matrix;
	normal_transform.invert().transpose();
	GLfloat diffuse_intensity[3], g_lightpos[3], g_viewpos[3], eyeOffset[3];
	switch (lightingMode) {
		case 0:
			setGLVec3(g_lightpos, directional_light.position);
			setGLVec3(diffuse_intensity, directional_light.diffuse_intensity);
			break;
		case 1:
			setGLVec3(g_lightpos, point_light.position);
			setGLVec3(diffuse_intensity, point_light.diffuse_intensity);
			break;
		case 2:
			setGLVec3(g_lightpos, spot_light.position);
			setGLVec3(diffuse_intensity, spot_light.diffuse_intensity);
			break;
	}
	setGLVec3(g_viewpos, main_camera.position);
	...
	glUniformMatrix4fv(iLocNormalTransform, 1, GL_FALSE, normal_transform.getTranspose());
	glUniform3fv(iLocDiffuseIntensity, 1, diffuse_intensity);
	glUniform3fv(iLocLightPos, 1, g_lightpos);
	glUniform3fv(iLocViewPos, 1, g_viewpos);
	glUniform1i(iLocligtingMode, lightingMode);
	glUniform1f(iLocShininess, shininess);
	glUniform1f(iLocCutoff, cos(spot_light.cutoff * PI / 180));
	glUniform1i(iLocIsPerPixel, isPerpixel);
	

	for (int i = 0; i < models[cur_idx].shapes.size(); i++) 
	{
		glBindVertexArray(models[cur_idx].shapes[i].vao);
		glUniform3f(iLocAmbient, models[cur_idx].shapes[i].material.Ka[0], models[cur_idx].shapes[i].material.Ka[1], models[cur_idx].shapes[i].material.Ka[2]);
		glUniform3f(iLocDiffuse, models[cur_idx].shapes[i].material.Kd[0], models[cur_idx].shapes[i].material.Kd[1], models[cur_idx].shapes[i].material.Kd[2]);
		glUniform3f(iLocSpecular, models[cur_idx].shapes[i].material.Ks[0], models[cur_idx].shapes[i].material.Ks[1], models[cur_idx].shapes[i].material.Ks[2]);
		...
		if (models[cur_idx].hasEye && models[cur_idx].shapes[i].material.isEye != 0)
		{
			Offset eyeOffset = models[cur_idx].shapes[i].material.offsets[models[cur_idx].cur_eye_offset_idx];
			glUniform2f(iLocEyeOffset, eyeOffset.x, eyeOffset.y);
		}
		else
		{
			glUniform2f(iLocEyeOffset, 0.0, 0.0);
		}

		
		if (magnification == mag_nearest)
			glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
		else
			glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
		
		if (minification == min_nearest)
		...
		switch (key)
		{
			...
			case GLFW_KEY_G:
				if (magnification == mag_nearest)
					...
					magnification = mag_nearest;
				break;
			case GLFW_KEY_B:
				if (minification == min_nearest)
					minification = linear_mipmap_linear;
				else
					minification = min_nearest;
				break;
			case GLFW_KEY_RIGHT:
...
void scroll_callback(GLFWwindow* window, double xoffset, double yoffset)
{
	...
	switch (cur_trans_mode)
	{
		...
		case LightEdit:
			switch (lightingMode) {
				case 0:
					directional_light.diffuse_intensity += Vector3(0.1 * (float)yoffset, 0.1 * (float)yoffset, 0.1 * (float)yoffset);
					break;
				case 1:
					point_light.diffuse_intensity += Vector3(0.1 * (float)yoffset, 0.1 * (float)yoffset, 0.1 * (float)yoffset);
					break;
				case 2:
					spot_light.cutoff += 10 * (float)yoffset;
					break;
			}
			break;
		case ShininessEdit:
			shininess += 20 * (float)yoffset;
			break;
	}
}
	...
	if (mouse_pressed) {
		if (starting_press_x < 0 || starting_press_y < 0) {
			...
			case LightEdit:
				switch (lightingMode) {
					case 0:
						directional_light.position.x += diff_x * (1.0 / 400.0);
						directional_light.position.y += diff_y * (1.0 / 400.0);
						break;
					case 1:
						point_light.position.x = diff_x * (1.0 / 400.0);
						point_light.position.y = diff_y * (1.0 / 400.0);
						break;
					case 2:
						spot_light.position.x = diff_x * (1.0 / 400.0);
						spot_light.position.y = diff_y * (1.0 / 400.0);
						break;
				}
				break;
			}
...
void LoadTexturedModels(string model_path)
{
	...
	for (int i = 0; i < materials.size(); i++)
	{
		...
		std::size_t eye_key = string(materials[i].diffuse_texname).find("Eye");
		material.isEye = 0;
		if (eye_key != std::string::npos) {
...
void initParameter()
{
	...
	lightingMode = 0;
	shininess = 64;
	directional_light.position = Vector3(1.0f, 1.0f, 1.0f);
	directional_light.diffuse_intensity = Vector3(1.0f, 1.0f, 1.0f);
	directional_light.cutoff = 30;

	point_light.position = Vector3(0.0f, 2.0f, 1.0f);
	point_light.diffuse_intensity = Vector3(1.0f, 1.0f, 1.0f);
	point_light.cutoff = 30;

	spot_light.position = Vector3(0.0f, 0.0f, 2.0f);
	spot_light.diffuse_intensity = Vector3(1.0f, 1.0f, 1.0f);
...
void setUniformVariables()
{
	...
	iLocNormalTransform = glGetUniformLocation(program, "normal_transform");
	iLocAmbient = glGetUniformLocation(program, "material.Ka");
	iLocDiffuse = glGetUniformLocation(program, "material.Kd");
	iLocSpecular = glGetUniformLocation(program, "material.Ks");
	iLocDiffuseIntensity = glGetUniformLocation(program, "diffuseIntensity");
	iLocLightPos = glGetUniformLocation(program, "lightPos");
	iLocViewPos = glGetUniformLocation(program, "viewPos");
	iLocIsPerPixel = glGetUniformLocation(program, "isPerPixel");
	iLocligtingMode = glGetUniformLocation(program, "lighting_mode");
	iLocShininess = glGetUniformLocation(program, "shininess");
	iLocCutoff = glGetUniformLocation(program, "cutoff");
	iLocTexture1 = glGetUniformLocation(program, "texture1");
	iLocisEye = glGetUniformLocation(program, "isEye");
	iLocEyeOffset = glGetUniformLocation(program, "eyeOffset");
...
int main(int argc, char **argv)
{
    ...
    // create window
	GLFWwindow* window = glfwCreateWindow(WINDOW_WIDTH, WINDOW_HEIGHT, "111065513 HW3", NULL, NULL);
    if (window == NULL)
    {
